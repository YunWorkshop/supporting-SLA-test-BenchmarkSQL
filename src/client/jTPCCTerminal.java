...
public class jTPCCTerminal implements jTPCCConfig, Runnable {
	...
	public jTPCCTerminal(String terminalName, int terminalWarehouseID, int terminalDistrictID, Connection conn,
			int dbType, int numTransactions, boolean terminalWarehouseFixed, int paymentWeight, int orderStatusWeight,
	...
	private void executeTransactions(int numTransactions) {
		...
		try {
			// new added,terminalName file has the sleeptime before each transaction. read
			// the file in a loop.
			String curdir = System.getProperty("user.dir");
			File file = new File(curdir + "/sleeptime/" + this.terminalName);
			FileReader fr = new FileReader(file);
			BufferedReader bufr = new BufferedReader(fr);
			bufr.mark((int) file.length() + 1);
			String s = null;
			for (int i = 0; (i < numTransactions || numTransactions == -1) && !stopRunning; i++) {
				if ((s = bufr.readLine()) != null) {
					int sleeptime = 0;
					try {
				...
				/*
				 * TPC/C specifies that each terminal has a fixed "home" warehouse. However,
				 * since this implementation does not simulate "terminals", but rather simulates
				 * "application threads", that association is no longer valid. In the case of
				 * having less clients than warehouses (which should be the normal case), it
				 * leaves the warehouses without a client without any significant traffic,
				...
				if (transactionType <= paymentWeight) {
					...
					try {
						...
						/*
						 * The old style driver does not have a delivery background queue, so we have to
						 * execute that part here as well.
						 */
